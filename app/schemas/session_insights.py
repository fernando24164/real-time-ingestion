...
class SessionInsightsResponse(BaseModel):
    ...
    avg_time_per_page: Optional[float] = Field(None, description="Average time spent per page in seconds")
    event_breakdown: Dict[str, int] = Field(..., description="Count of each event type")
    engagement_level: str = Field(..., description="Low, Medium, or High based on activity")
    
    @classmethod
    def from_session_insights(cls, insights: SessionInsights) -> "SessionInsightsResponse":
        """Convert raw session insights to API response format"""
        duration_minutes = insights.duration_seconds / 60 if insights.duration_seconds else 0
